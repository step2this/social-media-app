...
The pretty logs will grow over time. Consider rotating them:

```bash
# Manual rotation
mv apps/web/logs/app.log apps/web/logs/app.log.$(date +%Y%m%d)
touch apps/web/logs/app.log

# Or use logrotate (Linux/macOS)
# Create /etc/logrotate.d/social-media-app
/Users/shaperosteve/social-media-app/apps/web/logs/*.log {
    daily
    rotate 7
    compress
    missingok
