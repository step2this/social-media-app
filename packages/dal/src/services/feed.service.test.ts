...
/**
 * Mock DynamoDB client with in-memory storage
 * Supports PutCommand, QueryCommand, and BatchWriteCommand operations
 */
const createMockDynamoClient = () => {
  ...
  const handleQueryCommand = (command: MockDynamoCommand) => {
    ...
    if (KeyConditionExpression?.includes('PK = :pk AND begins_with(SK, :skPrefix)')) {
      ...
      const pk = ExpressionAttributeValues?.[':pk'] as string;
      results = Array.from(items.values()).filter(item => item.PK === pk);
    }
    ...
    results.sort((a, b) => {
      ...
      return ScanIndexForward === true ? aSK.localeCompare(bSK) : bSK.localeCompare(aSK);
    });
    ...
    return {
      ...
      LastEvaluatedKey: hasMore
        ? { PK: paginatedResults[paginatedResults.length - 1]?.PK, SK: paginatedResults[paginatedResults.length - 1]?.SK }
        : undefined
    };
  };

  const handleBatchWriteCommand = (command: MockDynamoCommand) => {
    ...
    const requests = requestItems[tableName] as Array<{ DeleteRequest?: { Key: { PK: string; SK: string } } }>;
