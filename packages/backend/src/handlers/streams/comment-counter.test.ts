    ...
    it('should increment commentsCount when COMMENT entity is inserted', async () => {
      const event = createMockDynamoDBStreamEvent({
        records: [
          {
            eventName: 'INSERT',
            keys: { PK: 'POST#post-123', SK: 'COMMENT#comment-456' },
            newImage: {
              PK: 'POST#post-123',
    ...
    it('should process multiple INSERT events in batch', async () => {
      const event = createMockDynamoDBStreamEvent({
        records: [
          ...
          {
            eventName: 'INSERT',
            keys: { PK: 'POST#post-123', SK: 'COMMENT#comment-2' },
            newImage: {
              PK: 'POST#post-123',
    ...
    it('should only process COMMENT entities', async () => {
      const event = createMockDynamoDBStreamEvent({
        records: [
          ...
          {
            eventName: 'INSERT',
            keys: { PK: 'POST#post-123', SK: 'COMMENT#comment-789' },
            newImage: {
              PK: 'POST#post-123',
    ...
    it('should continue processing if one record fails', async () => {
      ...
      const event = createMockDynamoDBStreamEvent({
        records: [
          ...
          {
            eventName: 'INSERT',
            keys: { PK: 'POST#post-456', SK: 'COMMENT#comment-2' },
            newImage: {
              PK: 'POST#post-456',
              SK: 'COMMENT#comment-2',
              entityType: 'COMMENT',
    ...
    it('should correctly extract postId from PK', async () => {
      const event = createMockDynamoDBStreamEvent({
        records: [
          {
            eventName: 'INSERT',
            keys: { PK: 'POST#abc-123-def-456', SK: 'COMMENT#comment-789' },
            newImage: {
