...
describe('HTTP Errors - Error Classes', () => {
  ...
  describe('ValidationError', () => {
    it('should create validation error with details', () => {
      ...
      const error = new ValidationError('Validation failed', 400, validationErrors);
...
describe('HTTP Errors - Classification Functions', () => {
  ...
  describe('extractErrorMessage', () => {
    ...
    it('should handle null error data', () => {
      const result = extractErrorMessage(null as any, 'Fallback');

      expect(result).toBe('Fallback');
    });

    it('should handle non-object error data', () => {
      const result = extractErrorMessage('string error' as any, 'Fallback');
  ...
  describe('createZodValidationError', () => {
    ...
    it('should format validation error with field paths', () => {
      ...
      expect(zodError).toBeDefined();
      const validationError = createZodValidationError(zodError!);
      const errors = validationError.validationErrors as any[];
