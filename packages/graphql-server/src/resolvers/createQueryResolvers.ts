...
export function createQueryResolvers(): QueryResolvers {
  return {
    me: async (_parent: unknown, _args: unknown, context: GraphQLContext, _info: GraphQLResolveInfo) => {
      const resolver = createMeResolver(context.container);
      return resolver(_parent, _args, context, _info);
    },

    profile: async (_parent: unknown, args: { handle: string }, context: GraphQLContext, info: GraphQLResolveInfo) => {
      const resolver = createProfileResolver(context.container);
      return resolver(_parent, args, context, info);
    },

    post: async (_parent: unknown, args: { id: string }, context: GraphQLContext, info: GraphQLResolveInfo) => {
