 ...
 * Builders can now use Zod schemas for runtime validation by setting this.schema.
 * This provides type safety, better error messages, and consistent validation.
 *
...
export abstract class BaseBuilder<TEntity, TConfig, TOutput> {
   ...
   * Subclasses can set this to enable schema-based validation
   */
  protected schema?: z.ZodSchema<TConfig>;
   ...
   * 1. Pre-build hooks
   * 2. Schema validation (if schema provided) or custom validation
  ...
  public async build(): Promise<TOutput> {
    ...
    try {
      ...
      // Validate configuration (schema-based if available, otherwise custom)
  ...
  /**
   * Validate configuration using Zod schema if provided, otherwise custom validation
   *
   * This method provides a unified validation interface that supports both:
   * - Schema-based validation (declarative, using Zod schemas)
   * - Custom validation (procedural, implementing validate() method)
   *
   * Priority: If schema is set, use it. Otherwise, fall back to custom validation.
   *
  ...
  protected async validateConfig(): Promise<ValidationResult> {
      ...
      try {
        ...
        if (error instanceof z.ZodError) {
          ...
          const errors = error.errors.map(err => ({
            ...
            message: err.message,
            value: err.path.reduce((obj: any, key) => obj?.[key], this.config),
          }));

          this.log('debug', `Schema validation failed with ${errors.length} errors`);
    ...
    // No schema provided - use custom validation
    this.log('debug', 'Using custom validation (no schema provided)');
